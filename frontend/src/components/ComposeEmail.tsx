...
export default function ComposeEmail({ onClose }: { onClose: () => void }) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const parsedEmails = lines.map(line => {
        // Assume first column or simple comma separation containing email
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/emails/schedule', {
        method: 'POST',
        headers: {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 overflow-y-auto max-h-screen">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            {recipients.length > 0 && (
              <p className="mt-2 text-sm text-green-600 font-medium">Valid emails detected: {recipients.length}</p>
            )}
          </div>
